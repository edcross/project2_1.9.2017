...
//        out.printf("Labels for image %s:\n", imagePath);
        ...
        final String FPATH = imagePath.toString();
        Filename archivo = new Filename(FPATH, '/', '.');

        nameOfTheFile = "/home/ed/Documents/VidriloTags/Vidrilo_GoogleCloudVision_original/Vidrilo_Sequence5_GoogleCloudVision/"+archivo.filename();

        /*end extractor*/
        for (EntityAnnotation label : labels) {
//            out.printf("\t%s (score: %.3f)\n", label.getDescription(), label.getScore());
            texto += label.getDescription().toString() + "-" + label.getScore().toString() + "\n";
        }
        if (labels.isEmpty()) {
            out.println("\tNo labels found.");
        }
        FileMethods.saveFile(texto, nameOfTheFile, true);
    }
    // [END run_application]
